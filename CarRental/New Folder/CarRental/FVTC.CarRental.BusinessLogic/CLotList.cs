...
namespace FVTC.CarRental.BusinessLogic
{
    public class CLotList
    {
        ...
        public void SelectLotsbyRow(string row)
        {
            CarRentalDataContext oDc = new CarRentalDataContext();

            //Select customers using LINQ
            var otblLots = from c in oDc.sp_GetLotByRow(row)
                                select c;

            oLotList = new List<CLot>();
            // Fill out Generic List of Customers "Customer" is a tablerow
            foreach (sp_GetLotByRowResult otblLot in otblLots)
            {
                CLot oLot = new CLot();
                //Pass in the LINQ Table Row to the class and load
                oLot.Load(otblLot);
